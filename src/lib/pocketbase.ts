...
// ── Promo token management ─────────────────────────────────────────────────

/** Creates a new promo record. Returns the created record. */
...
/** Returns existing promo for a buyer email + release slug, or null if none. */
export async function getPromoByEmailAndSlug(
  email: string,
  releaseSlug: string,
): Promise<PromoRecord | null> {
  const filter = encodeURIComponent(
    `recipient_email='${pbEscape(email)}' && release_slug='${pbEscape(releaseSlug)}'`,
  );
  try {
    const res = await fetch(
      `${PB_URL}/api/collections/promos/records?filter=${filter}&perPage=1`,
...
/** Records a Bandcamp sale ID so we don't re-process it on the next poll. */
export async function recordBandcampSale(saleId: string): Promise<void> {
  const adminToken = process.env.POCKETBASE_ADMIN_TOKEN;
  const headers: Record<string, string> = { 'Content-Type': 'application/json' };
  if (adminToken) headers['Authorization'] = `Bearer ${adminToken}`;

  await fetch(`${PB_URL}/api/collections/bandcamp_sales/records`, {
    method: 'POST',
    headers,
    body: JSON.stringify({ sale_id: saleId }),
  }).catch(() => {}); // fire-and-forget — collection may not exist yet
}

/** Returns true if this Bandcamp sale ID has already been processed. */
export async function bandcampSaleProcessed(saleId: string): Promise<boolean> {
  const filter = encodeURIComponent(`sale_id='${pbEscape(saleId)}'`);
  try {
    const res = await fetch(
      `${PB_URL}/api/collections/bandcamp_sales/records?filter=${filter}&perPage=1`,
      { headers: { 'Content-Type': 'application/json' } },
    );
